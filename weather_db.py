...
# Visual Crossing exports the city as "name" and the date as "datetime";
# the original bundled file renamed them to "City" and "Date". Accept both.
SOURCE_ALIASES = {"name": "City", "city": "City", "datetime": "Date", "date": "Date"}


def extract(csv_path: str | Path) -> pd.DataFrame:
    """Read the raw CSV and keep only the columns the model actually uses."""
    df = pd.read_csv(csv_path)
    df = df.rename(columns=lambda c: SOURCE_ALIASES.get(str(c).strip().lower(), c))
...
def _parse_dates(raw: pd.Series) -> pd.Series:
    """Parse dates from either supported source format, explicitly.

    Do NOT use pd.to_datetime(..., dayfirst=True) here. pandas infers one
    format from the first row; if that row is an ambiguous ISO date such as
    2025-04-09, it locks onto year-DAY-month, silently swaps day and month
    on every ambiguous row, and turns every day above 12 into NaT. On a
    Visual Crossing export that dropped 11 of 15 rows.

    The two formats we accept are structurally distinct (year first vs
    year last), so trying each explicitly is unambiguous, and the row-wise
    fillna means even a file mixing both formats parses correctly.
    """
    s = raw.astype(str).str.strip()
    iso = pd.to_datetime(s, format="%Y-%m-%d", errors="coerce")   # Visual Crossing
    dmy = pd.to_datetime(s, format="%d-%m-%Y", errors="coerce")   # original bundled file
...
def transform(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["obs_date"] = _parse_dates(df["Date"]).dt.strftime("%Y-%m-%d")
